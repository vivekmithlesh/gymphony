-- =============================================================================
-- APPLY: Payoneer international owner subscriptions (migration 20260705).
-- -----------------------------------------------------------------------------
-- Run this whole file in the Supabase SQL Editor (or `supabase db push`) to fix:
--   "Could not find the function public.app_create_intl_payment(...) in the
--    schema cache"  — the function simply was not applied to the live DB yet.
--
-- Idempotent + transactional — safe to re-run. Ends with `notify pgrst,
-- 'reload schema'` so the PostgREST schema cache refreshes automatically.
--
-- Prereqs (already live — the Indian UPI subscription flow depends on them):
--   app_config + is_platform_admin() (20260629), platform_support_* (20260630),
--   gym_settings plan columns + lockdown (20260621), activity_log (20260624).
-- Touches ONLY new international objects — Indian UPI, check-in and member
-- systems are not modified.
-- =============================================================================
...
-- ----------------------------------------------------------------------------
-- QUICK VERIFY (run in the SQL Editor after the COMMIT above).
-- NOTE: calling app_create_intl_payment() directly here raises "not authenticated"
-- because auth.uid() is NULL in the SQL Editor — verify by EXISTENCE instead, then
-- retest the real flow from the app (logged-in owner, Netherlands · Growth · monthly).
-- ----------------------------------------------------------------------------
-- 1) Function exists with the exact signature the frontend calls (p_tier, p_country, p_cycle):
--    select p.proname, pg_get_function_identity_arguments(p.oid) as args
--    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
--    where n.nspname = 'public' and p.proname = 'app_create_intl_payment';
--    -- expect args: p_tier text, p_country text, p_cycle text
--
-- 2) Table exists:
--    select to_regclass('public.international_payments');  -- expect: public.international_payments
--
-- 3) Force a schema-cache reload if the app still 404s the RPC:
--    notify pgrst, 'reload schema';
--    (or Dashboard → Project Settings → API → "Reload schema cache")
